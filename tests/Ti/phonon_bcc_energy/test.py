...
def phonopy_pre_process(cell, disp, supercell_matrix=None):
...
for disp in np.linspace(0.000001, 0.5, 50):
    D.append(disp)
    phonon = phonopy_pre_process(cell, disp, supercell_matrix=np.eye(3, dtype='intc'))
