...
(args, models, bulk_tests, default_analysis_settings) = analyze_start('interstitial_pes')
...
for model_name in sorted(models):
    if bool(data[model_name]["interstitial_pes"]) == True:
        E_x = data[model_name]["interstitial_pes"]["E_x"]
        E_xy = data[model_name]["interstitial_pes"]["E_xy"]

        E_x_shift = [ e - E_x[9] for e in E_x]
        E_xy_shift = [ e - E_xy[9] for e in E_xy]

        disp_list = data[model_name]["interstitial_pes"]["disp_list"]
...
plt.title(bulk_tests[0])
ax1.set_xlabel(r"Displacement (x) $(\AA)$")
ax1.set_ylabel("Energy (eV)")
ax2.set_xlabel(r"Displacement (x=y) $(\AA)$")
ax2.set_ylabel("Energy (eV)")
ax1.set_ylim((-0.025,0.10))
ax2.set_ylim((-0.025,0.10))
ax2.set_xlim((-0.065,0.065))
plt.legend()
plt.savefig("interstitial_pes.pdf", bbox_inches='tight')
